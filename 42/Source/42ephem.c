...
/* .. Earth's moon */
	Iw = World[EARTH].Sat[0];
	Eph = &World[Iw].eph;
	Eph2RV(Eph->mu, Eph->SLR,Eph->ecc,Eph->inc,Eph->RAAN,
		  Eph->ArgP,AbsTime-Eph->tp,Eph->PosN,Eph->VelN,&Eph->anom);
	World[Iw].PriMerAng = fmod(World[Iw].w*AbsTime,TwoPi);
	SimpRot(ZAxis,World[Iw].PriMerAng,World[Iw].CWN);
	MTxV(World[EARTH].CNH,Eph->PosN,rh);
	MTxV(World[EARTH].CNH,Eph->VelN,vh);
	for(i=0;i<3;i++) {
	  World[Iw].PosH[i] = rh[i] + World[EARTH].PosH[i];
	  World[Iw].VelH[i] = vh[i] + World[EARTH].VelH[i];
	}
